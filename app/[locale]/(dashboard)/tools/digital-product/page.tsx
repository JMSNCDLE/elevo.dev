...
import { Package, Send, Loader2, Bot, Lock, Crown } from 'lucide-react'
...
export default function DigitalProductPage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([{ role: 'assistant', content: "I'm the ELEVO Digital Product Creator — Galaxy exclusive. I help you create sellable digital products from your expertise. Ebooks, templates, guides, courses, lead magnets. What's your area of expertise?" }])
  ...
  const sendMessage = useCallback(async (msg?: string) => {
    ...
    try {
      const res = await fetch('/api/tools/digital-product', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: text, conversationHistory: messages }) })
  ...
  if (plan !== 'galaxy') return (<div className="p-6 max-w-2xl mx-auto"><div className="bg-gradient-to-b from-purple-900/20 to-dashCard border border-purple-500/20 rounded-2xl p-10 text-center"><Crown className="w-12 h-12 text-purple-400 mx-auto mb-4" /><div className="inline-block bg-purple-500/10 border border-purple-500/20 rounded-full px-3 py-1 text-[11px] font-bold text-purple-400 uppercase mb-4">Galaxy Exclusive</div><h1 className="text-2xl font-bold text-white mb-2">Digital Product Creator</h1><p className="text-dashMuted mb-6">Create sellable digital products: ebooks, templates, courses, guides.</p><Link href={`/${locale}/pricing`} className="inline-block px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-xl">Upgrade to Galaxy →</Link></div></div>)
  return (<div className="p-6 max-w-5xl mx-auto"><div className="flex items-center gap-3 mb-6"><div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-xl flex items-center justify-center"><Package className="w-5 h-5 text-white" /></div><div><h1 className="text-xl font-bold text-white">Digital Product Creator</h1><span className="text-[10px] font-bold text-purple-400 bg-purple-500/10 border border-purple-500/20 rounded-full px-2 py-0.5 uppercase">Galaxy</span></div></div>
